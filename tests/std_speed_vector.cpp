...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   std_speed_vector.cpp                               :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: aessakhi <aessakhi@student.42.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/01/17 19:43:37 by aessakhi          #+#    #+#             */
/*   Updated: 2023/01/17 19:47:06 by aessakhi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int main()
{
	...
	vector<pair<int, char> > vector_test;

	size_t	n_of_insert = 5000000;

	gettimeofday(&before, NULL);
	for (size_t i = 0; i < n_of_insert; i++)
		vector_test.push_back(make_pair(i, 'a'));
	std::cout << "Time to insert 5000000 pair: " << get_relative_ms(before) << " ms" << std::endl;
	
	vector<pair<int, char> >::iterator it = vector_test.begin();
	gettimeofday(&before, NULL);
	while (it != vector_test.end())
		vector_test.pop_back();
	std::cout << "Time to pop_back 5000000 pair: " << get_relative_ms(before) << " ms" << std::endl;
}
